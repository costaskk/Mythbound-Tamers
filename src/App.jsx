...
function MythboundTamersJRPGInner() {
  ...
  function buildSaveData(g = gameRef.current) {
    ...
    return { version: 12, savedAt: Date.now(), screen: safeScreen, storyIndex: g.storyIndex, player: g.player, party: g.party, storage: g.storage || [], active: g.active, seen: g.seen, dex: g.dex, clock: g.clock, muted: g.muted };
  }
  ...
  async function uploadSaveDataToCloud(saveData = buildSaveData(), show = true) {
    ...
    const cleanSave = JSON.parse(JSON.stringify({ ...migrated, version: 12, savedAt: Date.now() }));
    ...
    const payload = {
      ...
      save_data: cleanSave,
      save_version: cleanSave.version || 12,
...
function WorldScreen({ map, area, player, move, party, storage, seen, dex, setScreen, saveGame, clock, onObjectiveClick, objectiveMapFocus, clearObjectiveFocus }) {
  ...
  return <motion.div key="world" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`min-h-[calc(100vh-110px)] sm:min-h-[740px] p-3 sm:p-5 bg-gradient-to-br ${area?.bg || "from-slate-950 via-emerald-950 to-slate-950"}`}>
    <div className="flex flex-wrap justify-between gap-3 items-center mb-3">
      <div>
        <h2 className="text-sm sm:text-base font-black flex items-center gap-2"><Map className="w-7 h-7 text-cyan-200"/>Luminara Wilds</h2>
...
function AccountScreen({
  ...
  async function loadCloudSave() {
    try {
      ...
      await uploadSaveDataToCloud({ ...migrated, version: 12, savedAt: Date.now() }, false);
