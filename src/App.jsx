...
const DEFAULT_UPDATE_MANIFEST_URL = "https://costaskk.github.io/Mythbound-Tamers/update-manifest.json";
const UPDATE_MANIFEST_URL = import.meta.env.VITE_UPDATE_MANIFEST_URL || DEFAULT_UPDATE_MANIFEST_URL;
...
function normalizeUpdateManifest(data) {
  ...
  const apkUrl = data.apkUrl || data.apk_url || data.downloadUrl || data.download_url || data.url || "";
  return {
    version: String(data.version || data.versionName || data.tag || ""),
    versionCode: Number(data.versionCode || data.version_code || 0),
    notes: data.notes || data.changelog || data.description || "No update notes were provided.",
    apkUrl,
    webUrl: data.webUrl || data.releaseUrl || data.release_url || data.html_url || apkUrl,
    mandatory: Boolean(data.mandatory || data.required),
    publishedAt: data.publishedAt || data.published_at || data.date || "",
  };
}
function isUpdateNewer(manifest) {
  if (!manifest) return false;
  if (manifest.versionCode && Number(import.meta.env.VITE_APP_VERSION_CODE || 0) > 0) {
    return manifest.versionCode > Number(import.meta.env.VITE_APP_VERSION_CODE || 0);
  }
  return manifest.version && compareVersions(manifest.version, APP_VERSION) > 0;
}
function startApkInstallFromUrl(url) {
  if (!url) return false;
  try {
...
const BESTIARY = {
  ...
  glasscub: { name: "Glasscub", type: "Crystal", species: "Glass Cub", cry: "gling-grr!", stage: 1, evo: { to: "mirrursa", method: "Reach Lv.20 in Prism Ruins" }, base: [55, 20, 20, 18], skills: ["Crystal Shard", "Guard", "Fang Rush"], capture: 0.2, body: "bear", colors: ["#a5f3fc", "#f0abfc", "#0f172a"], lore: "A young cub with mirror-fur that reflects moonlight into tiny rainbows." },
  mirrursa: { name: "Mirrursa", type: "Crystal", species: "Mirror Bear", cry: "MIRR-URSA!", stage: 2, evo: { to: "prismursa", method: "Reach Lv.34 after Prism Key" }, base: [104, 36, 34, 26], skills: ["Crystal Shard", "Prism Nova", "Fang Rush"], capture: 0.06, body: "bear", colors: ["#67e8f9", "#f5d0fe", "#1e1b4b"], lore: "A reflective guardian that can split a charging enemy into harmless illusions." },
  prismursa: { name: "Prismursa", type: "Crystal", species: "Prism Bear Regent", cry: "PRISM-URSA!", stage: 3, base: [146, 52, 48, 32], skills: ["Crystal Shard", "Prism Nova", "Continental Slam"], capture: 0.022, body: "bear", colors: ["#22d3ee", "#f0abfc", "#020617"], lore: "A rare third-stage regent whose claws carve pure color into the air." },
  puddlepip: { name: "Puddlepip", type: "Aqua", species: "Rain Chick", cry: "pip-plip!", stage: 1, evo: { to: "rainroost", method: "Reach Lv.16 at Tideglass Flats" }, base: [38, 16, 11, 30], skills: ["Bubble Bite", "Gust Peck", "Healing Rain"], capture: 0.34, body: "bird", colors: ["#bae6fd", "#38bdf8", "#0f172a"], lore: "A tiny rain bird that hops from puddle to puddle after storms." },
  rainroost: { name: "Rainroost", type: "Aqua", species: "Storm Rooster", cry: "RAIN-ROOO!", stage: 2, base: [82, 32, 22, 43], skills: ["Bubble Bite", "Sky Dive", "Tidal Crush"], capture: 0.08, body: "bird", colors: ["#0ea5e9", "#e0f2fe", "#1e3a8a"], lore: "Its crest predicts rainfall and shines brighter when thunder is near." },
  dustrat: { name: "Dustrat", type: "Toxic", species: "Dust Rat", cry: "skrri-dust!", stage: 1, evo: { to: "plaguerat", method: "Reach Lv.18 at night" }, base: [44, 22, 10, 28], skills: ["Toxic Sting", "Fang Rush", "Mind Fog"], capture: 0.3, body: "roo", colors: ["#84cc16", "#57534e", "#1c1917"], lore: "A scrappy alley Mythling that sniffs out spoiled berries before they spread sickness." },
  plaguerat: { name: "Plaguerat", type: "Toxic", species: "Plague Rat", cry: "PLAGUE-SKRR!", stage: 2, base: [86, 39, 20, 42], skills: ["Toxic Sting", "Fang Rush", "Shadow Spiral"], capture: 0.075, body: "roo", colors: ["#65a30d", "#a855f7", "#020617"], lore: "It carries antidote herbs in its tail even though it looks terrifying." },
...
const ENCOUNTERS = {
  G: ["emberlynx", "aquapup", "leafawn", "voltoroo", "cloudfinch", "spriggeist", "spirikit", "bellimp", "bramblepup"],
  ...
  O: ["leafawn", "spriggeist", "orchidimp", "mistowl", "spirikit", "bellimp"],
  Q: ["pebbkit", "sandillo", "ironboar", "cindermole", "gearmite", "steelfang", "crysteel", "ferroach"],
  P: ["prismite", "gloomander", "starwhale", "aurorabbit", "lumifox", "gearmite", "crysteel", "glasscub", "mirrursa", "phantelope", "chimegeist", "glimmernewt", "radiantoad", "gemtoad", "opaloracle"],
  ...
  X: ["toxifrog", "venomire", "spirikit", "shadebat", "nightmoth", "chimegeist", "dustrat", "plaguerat"],
  ...
  "8": ["tidebug", "shellsurge", "coralisk", "aquapup", "neonsquid", "puddlepip", "rainroost"]
};
const TILE_NAMES = {
...
const AREA_DATA = {
  ...
  echoCaves: {
    ...
    encounters: { E: ["echopup","bellimp","howlitzer","chimegeist","mistowl"], V: ["gloomander","shadebat","spirikit","nightmoth"], G: ["cloudfinch","prismite"] },
  ...
  frostglassPeaks: {
    ...
    encounters: { H: ["snowkit","frostlynx","glacira","snowl","blizzowl","frostcub","glaciermaw","aurorabbit"], C: ["prismite","glimmernewt"] },
  ...
  verdantCanopy: {
    ...
    encounters: { J: ["mossmunk","cedarmunk","forestwarden","seedpuff","cottonstag","leafawn","florantler","spriggeist"], G: ["bramblepup","thornwolf","orchling"] },
  ...
  orchidCourt: {
    ...
    encounters: { O: ["orchling","orchidiva","orchidimp","bellimp","chimegeist","miragecalf","miragehart"], J: ["prismite","glimmernewt","radiantoad"] },
...
function resolveMove(attacker, defender, skillName, guarding = false) {
  ...
  const base = (((((2 * level) / 5 + 2) * power * offense / defense) / 50) + 2);
  const randomSpread = 0.9 + Math.random() * 0.14;
  const levelGap = Math.max(0.72, Math.min(1.28, 1 + ((level - (defender.level || 1)) * 0.018)));
  const damage = Math.max(2, Math.floor(base * stab * mult * critMult * guardMult * randomSpread * levelGap));
...
function moveSummary(skillName, mon, targetType, dex = null, enemy = null) {
  const sk = SKILLS[skillName] || SKILLS.Guard;
  if (sk.kind === "heal") return `Heal · ${ppText(mon, skillName)} · A100%`;
...
function MonsterModel({ mon, size = "large", flipped = false, faint = false, anim = "idle", silhouette = false }) {
  ...
  const eyeY = data.body === "bird" ? 92 : 104;
...
function MythboundTamersJRPGInner() {
  ...
  const [autoUpdateStatus, setAutoUpdateStatus] = useState("Update auto-check ready.");
  ...
  async function checkForAppUpdate({ silent = false } = {}) {
    if (!UPDATE_MANIFEST_URL) {
      if (!silent) setAutoUpdateStatus("No update manifest configured.");
      return null;
    }
    try {
      if (!silent) setAutoUpdateStatus("Checking for updates...");
      const res = await fetch(UPDATE_MANIFEST_URL, { cache: "no-store" });
      if (!res.ok) throw new Error(`Manifest HTTP ${res.status}`);
      const manifest = normalizeUpdateManifest(await res.json());
      if (!manifest) throw new Error("Invalid update manifest.");
      if (isUpdateNewer(manifest)) {
        setUpdatePrompt(manifest);
        setAutoUpdateStatus(`Update ${manifest.version || "new version"} available.`);
        if (!silent) sfx("success");
        return manifest;
      }
      setAutoUpdateStatus(`Latest version installed (${APP_VERSION}).`);
      if (!silent) setToast("You already have the latest version.");
      return manifest;
    } catch (e) {
      setAutoUpdateStatus(`Update check failed: ${e.message}`);
      if (!silent) setToast(`Update check failed: ${e.message}`);
      return null;
    }
  }
  function downloadAndInstallUpdate(manifest = updatePrompt) {
    const url = manifest?.apkUrl || manifest?.webUrl || manifest?.url;
    if (!url) {
      setAutoUpdateStatus("Update manifest has no APK URL.");
      setToast("No APK URL found in manifest.");
      return;
    }
    setAutoUpdateStatus("Starting APK download. Android may ask you to confirm installation.");
    setToast("Downloading update APK...");
    startApkInstallFromUrl(url);
  }
  useEffect(() => {
    const t = setTimeout(() => { checkForAppUpdate({ silent: true }); }, 1800);
    const interval = setInterval(() => { checkForAppUpdate({ silent: true }); }, 1000 * 60 * 60 * 6);
  ...
  async function uploadSaveDataToCloud(saveData = buildSaveData(), show = true) {
    ...
    const payload = { id: authUser.id, player_code: authUser.id, display_name: display, party_snapshot: saveData.party || [], dex_caught: Object.keys(saveData.dex?.caught || {}).filter((k) => saveData.dex.caught[k]).length, save_data: saveData, storage_snapshot: saveData.storage || [], inventory_snapshot: saveData.player || {}, save_version: saveData.version || 6, last_save_at: syncedAt, updated_at: syncedAt };
    const { error } = await supabase.from("mythbound_profiles").upsert(payload, { onConflict: "id" });
    if (error) throw error;
    setAccountProfile((old) => ({ ...(old || {}), ...payload }));
    setLastCloudSyncAt(syncedAt);
    setCloudSyncStatus(`Cloud synced online at ${formatOnlineSyncStamp(syncedAt)}`); if (show) setToast("Cloud save uploaded.");
  }
  ...
  function saveGame(show = true) { const data = buildSaveData(); localStorage.setItem(SAVE_KEY, JSON.stringify(data)); setHasSave(true); if (authUser) { setCloudSyncStatus("Cloud sync in progress..."); uploadSaveDataToCloud(data, false).catch((e) => { setCloudSyncStatus("Cloud sync failed"); setAccountStatus(`Cloud save failed: ${e.message}`); }); } else setCloudSyncStatus("Local only — sign in for cloud sync"); if (show) { setToast(authUser ? "Game saved locally + uploading to cloud." : "Game saved locally."); sfx("success"); } }
  ...
  function chooseStarter(id) { const starter = makeMon(id, 5); playCry(id); setParty([starter]); markCaught(starter); setPlayer((p) => ({ ...p, quest: "Reach Elder Nima in Grovepath village." })); setScreen("world"); setToast(`${displayName(starter)} joined your party.`); setRenameMon(starter.uid); setNickname(starter.name); setTimeout(() => saveGame(false), 50); }
  ...
  function playerUse(skillName) {
    ...
    if (skill.kind === "heal") {
      const heal = Math.min(me.maxHp - me.hp, skill.power + Math.floor(me.level * 3));
      me.hp += heal;
  ...
  function enemyTurn(enemySnapshot, playerGuard) {
    ...
    if (skill.kind === "heal") {
      const heal = Math.min(enemy.maxHp - enemy.hp, skill.power + enemy.level * 2);
      enemy.hp += heal;
...
function buildObjectivePayload(type, data, player, seen, dex) {
  ...
  if (type === "main") return {
    ...
    steps: [
      ...
      `Use the World Atlas if you need the route order.`,
  ...
  if (type === "side") return {
    ...
    steps: [
      `Travel to: ${data.area}`,
      data.done ? "This side objective is already complete." : "Progress this objective while exploring or catching Mythlings.",
...
function WorldScreen({ map, area, player, move, party, storage, seen, dex, setScreen, saveGame, clock, onObjectiveClick }) {
  ...
  return <motion.div key="world" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`min-h-[calc(100vh-110px)] sm:min-h-[740px] p-3 sm:p-5 bg-gradient-to-br ${area?.bg || "from-slate-950 via-emerald-950 to-slate-950"}`}>
    <div className="flex flex-wrap justify-between gap-3 items-center mb-3">
      <div>
        <h2 className="text-2xl sm:text-3xl font-black flex items-center gap-2"><Map className="w-7 h-7 text-cyan-200"/>Luminara Wilds</h2>
        <p className="text-slate-300 text-sm sm:text-base">Tap a tile for a pop-up. Use the floating D-pad to move. Pinch or +/- to zoom.</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <Badge className="bg-slate-800 text-cyan-100 border border-cyan-300/20 px-3 py-2"><TimeIcon className="w-4 h-4 mr-2"/>{timeString(clock)}</Badge>
        <Button onClick={() => setMapZoom((z)=>clampZoom(z - 0.15))} variant="secondary" className="rounded-xl font-bold px-3">−</Button>
        <Button onClick={() => setMapZoom(1)} variant="secondary" className="rounded-xl font-bold px-3">{Math.round(mapZoom * 100)}%</Button>
        <Button onClick={() => setMapZoom((z)=>clampZoom(z + 0.15))} variant="secondary" className="rounded-xl font-bold px-3">+</Button>
        <Button onClick={() => saveGame()} variant="secondary" className="rounded-xl font-bold hidden sm:inline-flex"><Save className="w-4 h-4 mr-2"/>Save</Button>
        <Button onClick={() => setScreen("dex")} variant="secondary" className="rounded-xl font-bold hidden sm:inline-flex"><BookOpen className="w-4 h-4 mr-2"/>Dex</Button>
        <Button onClick={() => setScreen("party")} variant="secondary" className="rounded-xl font-bold hidden sm:inline-flex"><PawPrint className="w-4 h-4 mr-2"/>Party</Button><Button onClick={() => setScreen("atlas")} variant="secondary" className="rounded-xl font-bold hidden sm:inline-flex"><Map className="w-4 h-4 mr-2"/>World</Button>
      </div>
    </div>

    <div className="lg:hidden mb-3 p-3 rounded-3xl bg-cyan-300/10 border border-cyan-200/20 text-sm text-cyan-50">
      Movement controls are floating above the bottom menu. The map scrolls, so you do not need the whole board visible at once.
    </div>

    <div className="mb-4">
      <ObjectivePanel player={player} seen={seen} dex={dex} party={party} storage={storage} clock={clock} compact onObjectiveClick={onObjectiveClick} />
    </div>

    <div className="grid xl:grid-cols-[1fr_330px] gap-4 items-start">
      <div
        className="overflow-auto rounded-3xl bg-black/25 border border-white/10 shadow-2xl p-3 overscroll-contain max-h-[44vh] sm:max-h-[62vh] xl:max-h-[620px]"
          ...
          >
            {map.map((row,y)=>row.padEnd(mapCols, "W").split("").map((t,x)=>{
              ...
              return <button
                ...
                key={`${x}-${y}`}
                className={`relative rounded-xl border flex items-center justify-center font-black shrink-0 overflow-hidden transition ${tileClass(t)} ${isAreaGate ? "ring-1 ring-cyan-200 shadow-md shadow-cyan-300/20" : ""} ${isSelected ? "ring-2 ring-white ring-offset-2 ring-offset-slate-950 z-10" : ""}`}
              ...
              >
                <span className="opacity-90 pointer-events-none leading-none">{label(t)}</span>{isAreaGate && <><span className="absolute right-0.5 top-0.5 w-1.5 h-1.5 rounded-full bg-cyan-200 border border-slate-950 shadow-sm"/><motion.span animate={{ opacity:[0.25,0.8,0.25], scale:[0.8,1.35,0.8] }} transition={{ duration:1.4, repeat:Infinity }} className="absolute inset-[4px] rounded-lg border border-cyan-100/60 pointer-events-none"/></>}
      ...
      <Card className="hidden lg:block rounded-3xl bg-white/5 border-white/10">
    ...
    <div className="hidden lg:grid max-w-4xl mx-auto mt-4 md:grid-cols-[1fr_260px] gap-3">
...
function TileInfoPopup({ selected, tile, x, y, close }) {
  return <motion.div
    ...
    className="lg:hidden fixed left-3 right-3 top-[88px] z-50 mx-auto max-w-md rounded-3xl bg-slate-950/95 border border-cyan-200/30 shadow-2xl shadow-cyan-500/20 backdrop-blur-xl p-4"
  >
...
function BattleScreen({ battle, playerMon, skills, playerUse, capture, selectedCaptureItem, setSelectedCaptureItem, usePotion, useStatusCure, usePPItem, run, player, party, active, setActive, anim, dex, clock, onBattleResultContinue }) {
  ...
  >
    ...
    <div className="relative w-full max-w-6xl mx-auto h-[31vh] min-h-[205px] sm:h-[50vh] sm:min-h-[370px] rounded-[1.25rem] overflow-hidden border border-white/10 shadow-2xl bg-gradient-to-b from-violet-900 via-fuchsia-950 to-slate-950 shrink-0" style={{ perspective: "950px", transformStyle: "preserve-3d" }}>
      ...
      <div className="absolute left-2 top-2 sm:left-5 sm:top-5 z-30 w-[64%] max-w-[330px] sm:w-[420px] sm:max-w-none">
      ...
      <div className="absolute right-2 bottom-2 sm:right-5 sm:bottom-5 z-30 w-[64%] max-w-[330px] sm:w-[430px] sm:max-w-none">
        <PokemonStatusBox mon={playerMon} title={displayName(playerMon)} align="right" showXp />
      </div>

      <div className="absolute right-[0%] top-[12%] sm:right-[7%] sm:top-[13%] z-20 scale-[0.62] sm:scale-100 origin-top-right pointer-events-none drop-shadow-2xl" style={{ transform: "translateZ(70px) rotateY(-8deg)", transformStyle: "preserve-3d" }}>
    ...
    <div className="w-full max-w-6xl mx-auto mt-1 rounded-2xl bg-slate-950/96 border-2 border-slate-700 shadow-2xl overflow-hidden flex-1 min-h-0">
      <div className="grid grid-cols-1 md:grid-cols-[0.75fr_1.25fr] gap-1.5 p-1.5 h-full">
        <div className="rounded-xl bg-white text-slate-800 border-2 border-slate-400 shadow-inner p-2 flex items-center justify-between gap-2 min-h-[48px] md:min-h-full">
          <div className="font-black text-sm sm:text-xl leading-snug flex-1">{battle.message}</div>
          
        </div>

        <div className="rounded-xl bg-white text-slate-800 border-2 border-slate-400 p-1.5 overflow-y-auto">
          {battle.turn === "player" ? <div className="grid grid-cols-2 gap-1 sm:gap-1.5">
            {suggestedMove ? <div className="col-span-2 rounded-lg bg-cyan-100 border border-cyan-300 text-slate-900 px-1.5 py-0.5 text-[10px] font-black leading-tight">Suggested move: {suggestedMove} · {moveSummary(suggestedMove, playerMon, enemy.type, dex, enemy)}</div> : <div className="col-span-2 rounded-lg bg-slate-100 border border-slate-300 text-slate-700 px-1.5 py-0.5 text-[10px] font-bold leading-tight">Effectiveness hidden: encounter or catch this Mythling to unlock type advantage hints in the Dex.</div>}
            {skills.map((s)=>{
              const sk=SKILLS[s];
              ...
              return <Button key={s} onClick={()=>playerUse(s)} disabled={!canUse} className={typeButtonClass(sk.type, !canUse)}>
                <Icon className="w-3.5 h-3.5 mr-1 shrink-0 drop-shadow"/>
                <span className="text-left leading-tight flex-1 min-w-0">
                  <span className="flex items-center gap-2 flex-wrap">
                    <b className="text-[11px] sm:text-sm">{s}</b>
                  ...
                  <br/>
                  <span className={`${sk.type === "Shadow" ? "text-slate-100" : "text-slate-700"} text-[8px] sm:text-[10px] font-semibold leading-tight`}>{moveSummary(s, playerMon, enemy.type, dex, enemy)}</span>
                </span>
              </Button>
            })}
            <div className="col-span-2 grid grid-cols-6 gap-1 pt-1 border-t border-slate-200">
              <div className="col-span-6"><select value={selectedCaptureItem} onChange={(e)=>setSelectedCaptureItem(e.target.value)} disabled={captureBlocked} className="w-full rounded-md border border-slate-300 bg-slate-100 px-1.5 py-1 text-[11px] font-bold">
                {Object.keys(CAPTURE_ITEMS).map((name)=><option key={name} value={name}>{name} x{captureCount(player,name)}</option>)}
              </select><div className="text-[10px] text-slate-600 mt-1 font-bold">Est. catch: {battle.mode === "wild" || battle.mode === "legend" ? `${selectedChance}%` : "N/A"}</div></div>
              <Button onClick={()=>capture(selectedCaptureItem)} disabled={captureBlocked || totalCaptureItems(player)<=0} className="rounded-md bg-cyan-300 hover:bg-cyan-200 text-slate-950 font-black text-[11px] px-1.5 py-1.5 col-span-2"><Backpack className="w-3.5 h-3.5 mr-1"/>Catch</Button>
              <Button onClick={usePotion} disabled={player.potions<=0||playerMon.hp===playerMon.maxHp} className="rounded-md bg-lime-200 hover:bg-lime-100 text-slate-950 font-black text-[11px] px-1.5 py-1.5 col-span-2"><Heart className="w-3.5 h-3.5 mr-1"/>Potion</Button>
              <Button onClick={useStatusCure} disabled={!playerMon.status || !hasAnyCureForStatus(player, playerMon.status)} className="rounded-md bg-amber-200 hover:bg-amber-100 text-slate-950 font-black disabled:opacity-40 text-[11px] px-1.5 py-1.5 col-span-2">Cure</Button>
              <Button onClick={usePPItem} className="rounded-md bg-violet-200 hover:bg-violet-100 text-slate-950 font-black text-[11px] px-1.5 py-1.5 col-span-2">PP</Button><Button onClick={run} disabled={battle.mode==="trainer" || battle.mode==="legend"} className={`rounded-md font-black text-[12px] px-1.5 py-2 disabled:opacity-40 col-span-2 border ${battle.mode==="wild" ? "bg-emerald-400 hover:bg-emerald-300 text-slate-950 border-emerald-200 shadow-md" : "bg-slate-300 hover:bg-slate-300 text-slate-700 border-slate-400"}`}>{battle.mode==="wild" ? "Run" : "No Run"}</Button>
            </div>
          </div> : battle.turn === "done" ? <div className="h-full min-h-[120px] flex flex-col items-center justify-center gap-3 text-slate-700 font-black text-lg sm:text-xl">
          ...
          </div> : <div className="h-full min-h-[120px] flex items-center justify-center text-slate-500 font-black text-lg sm:text-xl">Waiting...</div>}
        </div>
      </div>
    </div>
...
function BallThrow({ anim }) {
  const fx = anim?.fx || "capture";
  const item = anim?.captureItem || "Prism Capsule";
  const isSuccess = fx === "success";
  const isEscape = fx === "escape";
  const ballColor =
    item === "Ultra Prism" ? "from-yellow-300 via-white to-fuchsia-400" :
    item === "Great Prism" ? "from-blue-400 via-white to-cyan-300" :
    item === "Dusk Prism" ? "from-purple-700 via-slate-200 to-black" :
...
function ObjectiveDetailModal({ info, close }) {
  if (!info) return null;
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 z-[1650]">
    <motion.div initial={{y:24,scale:0.96}} animate={{y:0,scale:1}} exit={{y:24,scale:0.96}} className={`max-w-lg w-full rounded-[2rem] bg-gradient-to-br ${info.color || "from-cyan-200 to-fuchsia-300"} p-[2px] shadow-2xl`}>
      <div className="rounded-[1.9rem] bg-slate-950/95 p-5 text-white">
        ...
        <Button onClick={close} className="w-full rounded-2xl bg-cyan-300 text-slate-950 hover:bg-cyan-200 font-black py-5">Got it</Button>
      </div>
    </motion.div>
  </motion.div>;
}

function UpdateCenterScreen({ setScreen, checkForAppUpdate, updateStatus, downloadAndInstallUpdate }) {
  const [manifest, setManifest] = useState(null);
  const [checking, setChecking] = useState(false);
  async function manualCheck() {
    setChecking(true);
    const data = await checkForAppUpdate({ silent: false });
    setManifest(data);
    setChecking(false);
  }
  function install() {
  ...
  const newer = isUpdateNewer(manifest);
  return <motion.div key="update" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="min-h-[740px] p-4 sm:p-6 bg-gradient-to-br from-slate-950 via-cyan-950 to-indigo-950">
    <div className="flex justify-between items-start gap-3 mb-5">
      <div>
        <h2 className="text-3xl sm:text-4xl font-black flex items-center gap-2"><Upload className="w-8 h-8 text-cyan-200"/>Update Center</h2>
        <p className="text-slate-300 max-w-2xl">The app now auto-checks this manifest on startup and every few hours. If a newer APK exists, you get a popup.</p>
      </div>
      <Button onClick={()=>setScreen("world")} className="rounded-xl bg-cyan-300 text-slate-950 hover:bg-cyan-200 font-black">Back</Button>
    </div>
    <div className="grid lg:grid-cols-[1fr_380px] gap-4">
      <Card className="rounded-3xl bg-white/5 border-white/10"><CardContent className="p-5">
        <h3 className="text-2xl font-black mb-3">Current Version</h3>
        <InfoBox label="Installed" value={APP_VERSION}/>
        <div className="mt-3 rounded-2xl bg-black/25 border border-white/10 p-3 text-slate-200 text-sm">Manifest URL: <span className="break-all text-cyan-100">{UPDATE_MANIFEST_URL || "Not configured"}</span></div>
        <div className="grid sm:grid-cols-2 gap-2 mt-4">
          <Button onClick={manualCheck} disabled={checking} className="rounded-2xl bg-cyan-300 hover:bg-cyan-200 text-slate-950 font-black disabled:opacity-40">{checking ? "Checking..." : "Check Now"}</Button>
          <Button onClick={install} disabled={!manifest || !newer} className="rounded-2xl bg-lime-300 hover:bg-lime-200 text-slate-950 font-black disabled:opacity-40">Download & Install</Button>
        </div>
      </CardContent></Card>
      <Card className="rounded-3xl bg-white/5 border-white/10"><CardContent className="p-5">
        <h3 className="text-xl font-black mb-2">Update Status</h3>
        <div className="rounded-2xl bg-black/25 border border-white/10 p-3 min-h-[96px] text-slate-100">{updateStatus || "No check yet."}</div>
        {manifest && <div className="mt-3 text-sm text-slate-300 space-y-1">
          <div><b>Latest:</b> {manifest.version || "?"}</div>
          <div><b>Published:</b> {manifest.publishedAt || "?"}</div>
          <div><b>Required:</b> {manifest.mandatory ? "Yes" : "No"}</div>
          <div><b>Notes:</b> {manifest.notes || "No notes"}</div>
        </div>}
      </CardContent></Card>
    </div>
    <Card className="rounded-3xl bg-amber-300/10 border-amber-200/20 mt-4"><CardContent className="p-5 text-amber-50 text-sm">
      <b>Android note:</b> the app can automatically check and start the APK download. Android will still show its own install/security confirmation before replacing the app. Fully silent APK replacement is blocked by Android for normal apps.
    </CardContent></Card>
  </motion.div>;
}

function UpdateAvailableModal({ manifest, close, install }) {
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-[1800]">
    <motion.div initial={{y:26,scale:0.94}} animate={{y:0,scale:1}} exit={{y:26,scale:0.94}} className="max-w-xl w-full rounded-[2rem] border border-lime-200/40 bg-gradient-to-br from-slate-950 via-cyan-950 to-lime-950 shadow-2xl overflow-hidden">
      <div className="relative p-6">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_15%,rgba(34,211,238,.25),transparent_30%),radial-gradient(circle_at_80%_85%,rgba(190,242,100,.18),transparent_32%)]"/>
        <div className="relative">
          <div className="inline-flex items-center rounded-full bg-lime-300 text-slate-950 px-4 py-1 text-xs font-black mb-3">UPDATE AVAILABLE</div>
          <h2 className="text-3xl sm:text-5xl font-black text-white leading-tight mb-2">Mythbound v{manifest?.version || "new"}</h2>
          <p className="text-slate-100 text-base sm:text-lg leading-relaxed mb-4">{manifest?.notes || "A new version is ready to install."}</p>
          <div className="rounded-2xl bg-black/30 border border-white/10 p-3 text-sm text-slate-200 mb-5">
            Installed: <b>{APP_VERSION}</b><br/>
            Latest: <b>{manifest?.version || "?"}</b><br/>
            {manifest?.mandatory ? <span className="text-rose-200 font-bold">This update is marked as required.</span> : <span>You can install now or continue playing.</span>}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Button onClick={close} variant="secondary" className="rounded-2xl py-5 font-black bg-slate-100 text-slate-950 hover:bg-white">Later</Button>
            <Button onClick={install} className="rounded-2xl py-5 font-black bg-lime-300 text-slate-950 hover:bg-lime-200">Download & Install</Button>
          </div>
        </div>
      </div>
    </motion.div>
...
function MultiplayerScreen({ party, setParty, dex, player, setScreen, authUser, accountProfile, saveGame }) {
  ...
  async function submitBattleTurn() {
    try {
      ...
      const move = battleMove || skills(party[0] || { id: "emberlynx", level: 1 })[0] || "Guard";
...
function AccountScreen({
  ...
  async function uploadLocalSave() {
    try {
      ...
      setAccountStatus("Local save uploaded to your online account.");
    } catch (e) {
  ...
  async function loadCloudSave() {
    try {
      setBusy(true);
      const profile = await loadAccountProfile(authUser);
      if (!profile?.save_data) throw new Error("This account has no cloud save yet.");
      hydrateSaveData(profile.save_data, "cloud save");
      setAccountStatus("Cloud save loaded on this device.");
    } catch (e) {
        ...
        <CardContent className="p-5">
          ...
          {authUser ? <div className="space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              ...
              <Button onClick={loadCloudSave} disabled={busy} className="rounded-2xl bg-lime-300 hover:bg-lime-200 text-slate-950 font-black py-5">Load Cloud Save</Button>
            </div>
...
function MobileNav({ setScreen, saveGame, muted, setMuted, authUser }) {
  ...
  return <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 px-3 pb-3 pt-2 bg-slate-950/90 backdrop-blur-xl border-t border-white/10">
    ...
    <div className="grid grid-cols-5 gap-2 max-w-md mx-auto">
      ...
      <Button onClick={()=>setScreen("atlas")} variant="secondary" className={navButton}><Map className="w-5 h-5 mb-1"/>World</Button><Button onClick={()=>setScreen("help")} variant="secondary" className={navButton}><Sparkles className="w-5 h-5 mb-1"/>Help</Button><Button onClick={()=>setScreen("update")} variant="secondary" className={navButton}><Upload className="w-5 h-5 mb-1"/>Update</Button>
