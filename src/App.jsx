...
function MythboundTamersJRPGInner() {
  ...
  function buildSaveData(g = gameRef.current) {
    ...
    return { version: 13, savedAt: Date.now(), screen: safeScreen, storyIndex: g.storyIndex, player: g.player, party: g.party, storage: g.storage || [], active: g.active, seen: g.seen, dex: g.dex, clock: g.clock, muted: g.muted };
  }
  ...
  async function uploadSaveDataToCloud(saveData = buildSaveData(), show = true) {
    ...
    const cleanSave = JSON.parse(JSON.stringify({ ...migrated, version: 13, savedAt: Date.now() }));
    ...
    const payload = {
      ...
      save_data: cleanSave,
      save_version: cleanSave.version || 13,
...
function WorldScreen({ map, area, player, move, party, storage, seen, dex, setScreen, saveGame, clock, onObjectiveClick, objectiveMapFocus, clearObjectiveFocus }) {
  ...
  return <motion.div key="world" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`min-h-[calc(100vh-110px)] sm:min-h-[740px] p-1.5 sm:p-3 bg-gradient-to-br ${area?.bg || "from-slate-950 via-emerald-950 to-slate-950"}`}>
    <div className="relative mb-2 rounded-[1.5rem] overflow-hidden border border-cyan-200/20 bg-slate-950/70 shadow-2xl shadow-cyan-500/10">
      ...
      <div className="relative flex items-center justify-between gap-2 px-3 py-2">
        <div className="flex items-center gap-2 min-w-0">
          <Map className="w-5 h-5 text-cyan-200 shrink-0"/>
          <div className="truncate">
            <div className="text-[10px] uppercase tracking-[0.24em] text-cyan-100 font-black">Board</div>
            <div className="text-sm font-black text-white truncate">{area?.name || "Luminara Wilds"}</div>
          </div>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <Button onClick={() => setScreen("objectives")} variant="secondary" className="rounded-xl font-black px-2.5 py-1.5 text-xs bg-cyan-300 text-slate-950 hover:bg-cyan-200 border-cyan-100"><Sparkles className="w-3.5 h-3.5 mr-1"/>Goals</Button>
          <Badge className="bg-slate-800 text-cyan-100 border border-cyan-300/20 px-2 py-1 text-[10px]"><TimeIcon className="w-3 h-3 mr-1"/>{timeString(clock)}</Badge>
          <Button onClick={() => setMapZoom((z)=>clampZoom(z - 0.15))} variant="secondary" className="rounded-xl font-black px-2 py-1 text-xs">−</Button>
          <Button onClick={() => setMapZoom(1)} variant="secondary" className="rounded-xl font-black px-2 py-1 text-xs">{Math.round(mapZoom * 100)}%</Button>
          <Button onClick={() => setMapZoom((z)=>clampZoom(z + 0.15))} variant="secondary" className="rounded-xl font-black px-2 py-1 text-xs">+</Button>
        </div>
      </div>
    </div>
...
{objectiveFocusOnMap && <div className="mb-2 rounded-2xl bg-cyan-300/12 border border-cyan-200/30 p-2 flex flex-wrap items-center justify-between gap-2 shadow-xl shadow-cyan-500/10">
    ...
    <div className="relative">
      <div
        className="overflow-auto rounded-[2rem] bg-black/35 border border-white/10 shadow-2xl p-2 sm:p-3 overscroll-contain max-h-[calc(100dvh-160px)] sm:max-h-[calc(100dvh-150px)] xl:max-h-[calc(100dvh-150px)] min-h-[calc(100dvh-250px)]"
...
function TileInfoPopup({ selected, tile, x, y, close }) {
  return <motion.div
    ...
    className="fixed left-3 right-3 top-[78px] z-50 mx-auto max-w-md rounded-3xl bg-slate-950/95 border border-cyan-200/30 shadow-2xl shadow-cyan-500/20 backdrop-blur-xl p-4"
  >
...
function AccountScreen({
  ...
  async function loadCloudSave() {
    try {
      ...
      await uploadSaveDataToCloud({ ...migrated, version: 13, savedAt: Date.now() }, false);
